...
function collectUtilityHints($) {
  ...
  $("#parcelDetails_XFOBTable tr[bgcolor]").each((_, tr) => {
    ...
    if (analysis.flags.hasSeptic) hasSeptic = true;
    if (analysis.flags.hasWell) hasWell = true;
  });
  return { hasSeptic, hasWell };
}

function buildUtilityObject(hints) {
  const { hasSeptic, hasWell } = hints;
...
function main() {
  ...
  const util = buildUtilityObject(hints);
