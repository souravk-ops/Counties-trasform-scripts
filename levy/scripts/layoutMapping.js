...
function buildLayoutsFromBuildings(buildings) {
  ...
  buildings.forEach((b) => {
    ...
    // For now, let's assume 0 bedrooms if not found, and use "Baths" for bathrooms.
    totalBeds += toInt(b["Bedrooms"]); // This will likely be 0 based on the sample HTML
    totalBaths += toInt(b["Baths"]); // This is present in the sample HTML
  });
