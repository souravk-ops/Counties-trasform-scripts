...
const CURRENT_OWNER_SELECTOR = "#ctlBodyPane_ctl01_mSection .sdw1-owners-container > div:first-child > div:first-child a";
const SALES_TABLE_SELECTOR = "#ctlBodyPane_ctl11_ctl01_gvwList tbody tr";
...
function extractSalesOwnersByDate($) {
  ...
  rows.each((i, tr) => {
    ...
    const grantee = txt(tds.eq(tds.length - 1).text()); // Grantee is the last td
    if (grantee) {
      if (!map[dateStr]) map[dateStr] = [];
      map[dateStr].push(grantee);
    }
    const grantor = txt(tds.eq(tds.length - 2).text()); // Grantor is the second to last td
