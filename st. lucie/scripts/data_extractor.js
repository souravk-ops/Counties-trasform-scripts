...
async function resolveRelationshipEndpointWithFallback(
  ...
  const endpointKind = detectRelationshipEndpointKind(endpoint);
  if (endpointKind === "address") {
    const created = await createOrReuseAddressEntity(
      endpoint,
      entityMap,
      existingFiles,
      autoEntityCounters,
  ...
  } else if (endpointKind === "person") {
    const created = await createOrReusePersonEntity(
      endpoint,
      entityMap,
      existingFiles,
      autoEntityCounters,
...
function buildAddressOneOfPayload(source) {
  if (!source || typeof source !== "object") return null;

  const result = {};

  const normalizeCommonString = (value) => {
    if (value == null) return null;
    if (typeof value === "string") {
      const cleaned = textClean(value);
      return cleaned || null;
  ...
  const normalizeStructuredString = (key, value) => {
    if (value == null) return null;
    if (typeof value !== "string") return value;
    let cleaned = textClean(value);
    if (!cleaned) return null;
  ...
  for (const key of ADDRESS_METADATA_KEYS) {
    if (!Object.prototype.hasOwnProperty.call(source, key)) continue;
    const normalized = normalizeCommonString(source[key]);
  ...
  if (Object.prototype.hasOwnProperty.call(source, "request_identifier")) {
    const normalized = normalizeCommonString(source.request_identifier);
    if (typeof normalized === "string") requestIdentifier = normalized;
  } else if (
    Object.prototype.hasOwnProperty.call(source, "requestIdentifier")
  ) {
    const normalized = normalizeCommonString(source.requestIdentifier);
    if (typeof normalized === "string") requestIdentifier = normalized;
  }
  ...
  for (const key of STRUCTURED_ADDRESS_FIELDS) {
    if (!Object.prototype.hasOwnProperty.call(source, key)) continue;
    const normalized = normalizeStructuredString(key, source[key]);
    if (normalized == null) continue;
    structured[key] = normalized;
  }
  ...
  const directUnnormalizedCandidates = [
    ...
    Object.prototype.hasOwnProperty.call(source, "full_address")
      ? source.full_address
      : null,
    Object.prototype.hasOwnProperty.call(source, "address")
      ? source.address
      : null,
    Object.prototype.hasOwnProperty.call(source, "site_address")
      ? source.site_address
      : null,
    Object.prototype.hasOwnProperty.call(source, "mailing_address")
...
function ensureExclusiveAddressPayloadForWrite(payload, preferMode = "unnormalized") {
  ...
  if (hasStructured && hasUnnormalized) {
    if (normalizedMode === "structured") {
      ...
      if (normalizedValue) {
        working.unnormalized_address = normalizedValue;
      } else {
        delete working.unnormalized_address;
      }
    }
  ...
  for (const [key, value] of Object.entries(working)) {
    ...
    if (typeof value === "string") {
      ...
      if (key === "city_name" || key === "state_code") {
        ...
        working[key] = trimmed.replace(/\s+/g, "");
      } else if (key === "unnormalized_address") {
        working[key] = trimmed;
      } else {
        working[key] = trimmed;
      }
  ...
  if (
    ...
    const trimmed = working.unnormalized_address.trim();
    if (!trimmed) {
      delete working.unnormalized_address;
    } else {
      working.unnormalized_address = trimmed;
    }
  }
...
async function enforceAddressFileOneOf(
  ...
  const exclusivePayload =
    ensureExclusiveAddressPayloadForWrite(normalizedPayload, normalizedMode) ??
    ensureExclusiveAddressPayloadForWrite(
      normalizedPayload,
      normalizedMode === "structured" ? "unnormalized" : "structured",
    );

  if (!exclusivePayload || typeof exclusivePayload !== "object") {
    await fsp.unlink(filePath).catch(() => {});
    return null;
  }
...
async function normalizeAddressFileForSchema(
  fileName,
  { preferStructured = false, fallbackUnnormalized = null } = {},
) {
  if (!fileName || typeof fileName !== "string") return null;
  ...
  let requestIdentifier = null;
  if (Object.prototype.hasOwnProperty.call(parsed, "request_identifier")) {
    const rawId = parsed.request_identifier;
    if (typeof rawId === "string") {
      const trimmed = rawId.trim();
      if (trimmed) requestIdentifier = trimmed;
    } else if (rawId != null) {
      requestIdentifier = rawId;
    }
  }
  ...
  let output = null;
  if (preferStructured && hasStructured) {
    output = { ...metadata, ...structuredCandidate };
  }

  if (!output && normalizedUnnormalized) {
    output = {
      ...metadata,
      unnormalized_address: normalizedUnnormalized,
    };
...
async function enforceFinalAddressSchema(
  fileName,
  { preferStructured = false, fallbackUnnormalized = null } = {},
) {
  if (!fileName || typeof fileName !== "string") return null;
  ...
  if (
    ...
    (!preferStructured || !hasStructured)
  ) {
    output.unnormalized_address = normalizedUnnormalized;
  } else if (hasStructured) {
    ...
    for (const key of STRUCTURED_ADDRESS_FIELDS) {
      ...
      const rawValue = structuredCandidate[key];
      if (rawValue == null) {
        continue;
      }
      if (typeof rawValue === "string") {
        ...
        cleanedStructured[key] = trimmed;
      } else {
        cleanedStructured[key] = rawValue;
      }
    }
    output = { ...output, ...cleanedStructured };
  } else if (normalizedUnnormalized) {
    output.unnormalized_address = normalizedUnnormalized;
  } else {
    await fsp.unlink(filePath).catch(() => {});
...
async function normalizePersonFileForSchema(fileName) {
  ...
  const normalizedLast =
    normalizeNameToPattern(parsed.last_name, PERSON_NAME_PATTERN) ||
    fallbackName;
  const normalizedFirst =
    normalizeNameToPattern(parsed.first_name, PERSON_NAME_PATTERN) ||
    fallbackName;
  let normalizedMiddle = null;
  if (
    Object.prototype.hasOwnProperty.call(parsed, "middle_name") &&
  ...
  if (!PERSON_NAME_PATTERN.test(sanitized.last_name)) {
    sanitized.last_name = fallbackName;
  }
  if (!PERSON_NAME_PATTERN.test(sanitized.first_name)) {
    sanitized.first_name = fallbackName;
  }
  if (
    sanitized.middle_name != null &&
    !PERSON_MIDDLE_NAME_PATTERN.test(sanitized.middle_name)
  ) {
    sanitized.middle_name = null;
  }
...
function buildFinalAddressOutput(payload, options = {}) {
  if (!payload || typeof payload !== "object") return null;
...
async function performExtraction() {
  ...
  const addressMetadataSourcesForOutput = [
  ...
  const structuredForOutput = sanitizedStructuredCandidate
    ? pickStructuredAddress(sanitizedStructuredCandidate)
    : null;

  const hasStructuredForOutput =
    structuredForOutput &&
    STRUCTURED_ADDRESS_REQUIRED_KEYS.every(
      (key) =>
        typeof structuredForOutput[key] === "string" &&
  ...
  if (!isMulti) {
    ...
    $("article#property-identification table.container tr").each((i, tr) => {
      const th = textClean($(tr).find("th").text());
      const td = textClean($(tr).find("td").text());
      if (/Land Use Code/i.test(th)) landUseCodeText = td;
    });

    // Building Type from HTML (kept for completeness)
    ...
    propertyOut = {
      ...
      property_type: mappedPropertyDetails.property_type || "LandParcel",
  ...
  if (!isMulti) {
                ...
                if (mailingSchemaSafe) {
                  ...
                  if (enforcedMailing) {
                    ...
                    if (normalizedMailing) {
                      ...
                      const harmonizedMailing =
                        await normalizeAddressFileForSchema(
                          "mailing_address.json",
                          {
                            preferStructured: preferMailingStructuredForWrite,
                      ...
                      if (harmonizedMailing) {
                        const finalizedMailing =
                          await enforceFinalAddressSchema(
                            "mailing_address.json",
                            {
                              preferStructured:
                                preferMailingStructuredForWrite,
    ...
    for (const record of ownerRecords.values()) {
      ...
      const normalizedPersonForFile =
        await normalizePersonFileForSchema(fileName);
      if (!normalizedPersonForFile) {
        await fsp.unlink(path.join("data", fileName)).catch(() => {});
        personIdx -= 1;
        await promoteToCompany(validationFallback);
        continue;
