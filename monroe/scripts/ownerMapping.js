...
const CURRENT_OWNER_SELECTOR =
  "#ctlBodyPane_ctl03_ctl01_lstDeed_ctl00_sprDeedName_lnkUpmSearchLinkSuppressed_lblSearch"; // Updated selector
const SALES_TABLE_SELECTOR = "#ctlBodyPane_ctl09_ctl01_gvwList tbody tr"; // Updated selector
...
function extractSalesOwnersByDate($) {
  ...
  rows.each((i, tr) => {
    ...
    // Grantor is the 9th column (index 8)
    const grantor = txt(tds.eq(8).text()); // Updated index for Grantor
    if (grantor) priorOwners.push(grantor);

    // Grantee is the 10th column (index 9)
    const grantee = txt(tds.eq(9).text()); // Updated index for Grantee
    if (grantee) {
      if (!map[dateStr]) {
        map[dateStr] = [];
      }
