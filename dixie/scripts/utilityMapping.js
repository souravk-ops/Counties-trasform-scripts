...
function mapCoolingSystem(value) {
  ...
  if (v.includes("CENTRAL") || v.includes("DUCTED") || (!isNaN(Number(v)) && Number(v) > 1)) return "CentralAir";
...
function mapUtilityValues($, buildings) {
  ...
  buildings.forEach((b) => {
    cooling_system_type = mapCoolingSystem(b["Percent Air Conditioned"]);
    heating_system_type = mapHeatingSystem(b["Heat Type"]);
    heating_fuel_type = mapHeatingFuel(b["Heat Fuel"]);
    
    solar_panel_present = (b["Solar Hot Water"] || "").toUpperCase() === "YES";
...
function buildUtilityRecord($, parcelId, buildings) {
  ...
  return {
    ...
    heating_fuel_type: mapped.heating_fuel_type,
    ...
    solar_panel_present: mapped.solar_panel_present,
