...
function main() {
  ...
  function mapPropertyType(useCode) {
    ...
    if (uc.includes("MULTI-FAMILY") || uc.includes("000300")) return "MultipleFamily";
    if (uc.includes("SINGLE FAMILY") || uc.includes("000100")) return "SingleFamily";
    if (uc.includes("CONDO")) return "Condominium";
    if (uc.includes("DETACHED CONDO")) return "DetachedCondominium";
    if (uc.includes("NON-WARRANTABLE CONDO")) return "NonWarrantableCondo";
    if (uc.includes("VACANT") || uc.includes("LAND")) return "VacantLand";
    if (uc.includes("DUPLEX") || uc.includes("2 UNIT")) return "Duplex";
    if (uc.includes("TOWNHOUSE") || uc.includes("TOWNHOME")) return "Townhouse";
    if (uc.includes("APARTMENT")) return "Apartment";
    if (uc.includes("MOBILE HOME") || uc.includes("MANUFACTURED HOUSING")) return "MobileHome";
    if (uc.includes("MANUFACTURED SINGLE")) return "ManufacturedHousingSingleWide";
    if (uc.includes("MANUFACTURED MULTI")) return "ManufacturedHousingMultiWide";
    if (uc.includes("MODULAR")) return "Modular";
    if (uc.includes("COOPERATIVE") || uc.includes("CO-OP")) return "Cooperative";
    if (uc.includes("PUD")) return "Pud";
    if (uc.includes("TIMESHARE")) return "Timeshare";
    if (uc.includes("2 UNITS")) return "2Units";
    if (uc.includes("3 UNITS")) return "3Units";
    if (uc.includes("4 UNITS")) return "4Units";
    if (uc.includes("RETIREMENT")) return "Retirement";
    if (uc.includes("COMMON ELEMENTS")) return "ResidentialCommonElementsAreas";
    if (uc.includes("MULTI-FAMILY") && uc.includes(">10")) return "MultiFamilyMoreThan10";
    if (uc.includes("MULTI-FAMILY") && uc.includes("<10")) return "MultiFamilyLessThan10";
    return null;
  }
  ...
  function extractAndWriteTaxData($) {
    ...
    years.forEach((year, index) => {
      ...
      const taxData = {
        source_http_request: {
          method: "GET",
          url: seed.source_url || "https://example.com/tax-data"
        },
        request_identifier: `tax_${year}_${Date.now()}`,
        tax_year: parseInt(year),
        property_land_amount: parseCurrencyToNumber(landValues[index]) || null,
        property_building_amount: parseCurrencyToNumber(buildingValues[index]) || null,
        property_market_value_amount: parseCurrencyToNumber(marketValues[index]) || null,
        property_assessed_value_amount: parseCurrencyToNumber(assessedValues[index]) || null,
        property_taxable_value_amount: parseCurrencyToNumber(taxableValues[index]) || null,
  ...
  salesRows.each((i, tr) => {
    ...
    const purchase_price_amount = parseCurrencyToNumber(priceStr) || 0;
    ...
    const salesObj = {
      source_http_request: {
        method: "GET",
        url: seed.source_url || "https://example.com/sales-data"
      },
      request_identifier: `sales_${salesIndex}_${Date.now()}`,
    ...
    const deedObj = {
      source_http_request: {
        method: "GET",
        url: seed.source_url || "https://example.com/deed-data"
      },
      deed_type: deedType
    };
